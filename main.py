...
def get_data():
    ...
    for date in html_with_dates:
        ...
        if '<a' in date_clean and 'résumé' not in date_clean and date_clean[0] != '→':
            print(date_clean)
            date_clean_split = date_clean.split('<')
            date_clean = date_clean_split[0]
            date_clean_split_event = date_clean_split[1].split('>')
            date_clean += date_clean_split_event[1]
        if '<' not in date_clean and 'Voir aussi' not in date_clean:
            ...
            event_clean_lower_case = event.replace('.', '')
...
def split_period(character, date, event, data):
    ...
    new_date_begin = {
        "year": years[0],
        "event": 'Début ' + event[0].lower() + event[1:]
    }
    new_date_end = {
        "year": years[1],
        "event": 'Fin ' + event[0].lower() + event[1:]
    }
...
# TODO 907 : Chine : le dernier empereur Tang est détrôné. deal with the punctuaction in the middle -> DONE
# TODO Bug dates with 2 numbers av JC -> DONE
